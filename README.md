...
`/quota-cc` 按《2026 国赛名额分配规则 v1》展示五类名额。计入预选赛、2025 总决赛奖励 207 个及已知承办和出题奖励 79 个，省赛和邀请赛奖励、外卡暂不计入。承办奖励公布容量为 91 个：论坛 2 个、冬季会议 1 个暂不计；网络赛出题 6 个、高职赛承办 3 个待补充。上届总决赛承办方南阳理工学院计入 4 个；荆州站的 8 个承办奖励全部分配给长江大学，武汉大学不计该项奖励；南京大学出题另获 8 个。

预选赛按前 240 所有有效队伍的学校及前 326 支有效队伍分配。每校有效队伍上限为 `max(0, min(6, 12 - 总决赛奖励 - 承办奖励))`；奖励与首轮达到 12 的学校退出次轮。按规则 B 校示例截去超过每校 12 个上限的次轮名额，暂不递补，实际预选赛计入数可能少于公布容量 566 个。截线并列暂按源榜顺序，不扩容，并在页面提示。

页面使用固定榜单快照，显示获取时间及比赛阶段。更新网络赛和总决赛数据后重新构建即可发布新快照：

```sh
npm run update:ccpc-ranking
npm run test:ccpc-quota
npm run build
```

更新脚本校验榜单、剔除打星队伍，并核对总决赛 107 所学校 / 207 个奖励；失败时保留原快照。学校按源榜名称精确匹配，保留仅参加总决赛的学校。快照不存储队员姓名。
