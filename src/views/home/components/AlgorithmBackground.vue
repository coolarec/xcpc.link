...
const backgroundRows = ref<BackgroundRow[]>([])

let context: gsap.Context | undefined
let observer: IntersectionObserver | undefined
let backgroundTweens: gsap.core.Tween[] = []
let isMounted = false
...
onMounted(async () => {
  ...
  const algorithmWords = await fetchAlgorithmWords()
  backgroundRows.value = Array.from({ length: 12 }, (_, rowIndex) => ({
  ...
  context = gsap.context(() => {
    gsap.utils.toArray<HTMLElement>('.algorithm-word').forEach((word, index) => {
